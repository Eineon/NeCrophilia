...
**执行：** FA　｜　**消耗：** X SP<br>
———————————————————————————————<br>
选择1个<负伤>为2级或更低的部位，恢复X点HP。妳也可以选择多个符合条件的部位，随后拆分X为这些部分恢复HP。<br>
<span style="font-family: Wingdings">v</span> **特殊** &ensp;———————————————————————————<br>
这个动作在每个冲突场景中最多只能使用1次。<br>
如果在长休中使用此动作，则此动作会在完成长休时进入「清算阶段」。此时妳还可以为<负伤>为3级的部位恢复HP，并且在为<负伤>为2级或更低的部位恢复HP时，不需要消耗SP。
...
**执行：** TA　｜　**消耗：** 1+ AP<br>
———————————————————————————————<br>
妳执行1次【行进】。每多消耗1点AP，因此执行【行进】时，妳的Mov就会获得2点加值。<br>
<span style="font-family: Wingdings">v</span> **特殊** &ensp;———————————————————————————<br>
