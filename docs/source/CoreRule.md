...
&emsp;属性值最低只能被降至0。体质和感悟之外的属性值被降至0时，角色会陷入<昏迷>。体质值被降至0时，角色会立即<解体>。智慧值被降至0后，角色的「认知阶层」会变为「无认知」。
...
- **劣等**  
	...
	如果角色的智慧属性为劣等阶层，她的「认知阶层」会变为「低认知」。<hr>
- **次等**  
	数值为3~6的属性处于这个阶层。使用劣等属性进行技能检定时，总是会受到2点减值。<hr>
- **标准**
	数值为7~14的属性处于这个阶层。如果角色的智慧属性为标准或更高阶层，她的「认知阶层」会变为「高认知」。<hr>
- **高等**
...
&emsp;一部分动作需要在使用时消耗HP。产生的消耗会使HP低于下限的效果会使用失败。
...
- **全力：** 全力动作会消耗大量的时间。使用全力动作的期间，角色不能使用主要、次要和其他全力动作 (包括将这些类型的动作作为其他类型使用，例如【截击】)，否则将打断正在使用的全力动作。
- **自由：** 自由动作可以在任何时候使用，无论在角色的回合内，还是回合外。
- **即时：** 即时动作与自由动作类似，不过在使用即时动作后，角色无法再使用任何即时动作，直到她的下个回合开始。所有即时动作都具有「触发」条目。<hr>
